...
// draw latency range in ms - from new content/keypress/etc until drawing. within this range, st draws when content stops arriving (idle). mostly it's near minlatency, but it waits longer for slow updates to avoid partial draw. low minlatency will tear/flicker more, as it can "detect" idle too early.
...
// interval (in milliseconds) between each successive call to ximspot. This improves terminal performance while not reducing functionality to those whom need XIM support.
int ximspot_update_interval = 1000;

// thickness of underline and bar cursors
static unsigned int cursorthickness = 2;

// 1: render most of the lines/blocks characters without using the font for perfect alignment between cells (U2500 - U259F except dashes/diagonals). Bold affects lines thickness if boxdraw_bold is not 0. Italic is ignored.
...
// spaces per tab
//  When you are changing this value, don't forget to adapt the »it« value in the st.info and appropriately install the st.info in the environment where you use this st version.
  
// it#$tabspaces,
// Secondly make sure your kernel is not expanding tabs. When running `stty -a` »tab0« should appear. You can tell the terminal to not expand tabs by running following command:
